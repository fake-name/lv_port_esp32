...
// #define RA8876_SPI_SPEED (20 * 1000 * 1000)
#define RA8876_SPI_SPEED (1 * 1000 * 1000)
...
class RA8876
{
	private:
		...
		uint32_t oscClock;   // OSC clock (external crystal) frequency in kHz

		PllParams memPll;   // MCLK (memory) PLL parameters
		PllParams corePll;  // CCLK (core) PLL parameters
		PllParams scanPll;  // SCLK (LCD panel scan) PLL parameters

		SdramInfo *sdramInfo;

		DisplayInfo *displayInfo;

		ExternalFontRomInfo fontRomInfo;

		uint16_t textColor;
		int      textScaleX;
		int      textScaleY;

		enum FontSource fontSource;
		...
		void write_reg(uint8_t reg, uint8_t x);
		void write_reg16(uint8_t reg, uint16_t x);
		void write_reg16bbp(uint8_t reg, uint16_t x);
		uint8_t read_reg(uint8_t reg);
		uint16_t read_reg16(uint8_t reg);

		inline void wait_write_fifo(void)
		{
			while (this->read_status() & 0x80);
		};
		inline void wait_task_busy(void)
		{
			while (this->read_status() & 0x08);
		};

		bool calc_pll_params(uint32_t targetFreq, int kMax, PllParams *pll);
		bool calc_clocks(void);
		void dump_clocks(void);

		bool init_pll(void);
		bool init_memory(SdramInfo *info);
		bool init_display(void);

		// Font utils
		uint8_t internal_font_encoding(enum FontEncoding enc);
		...
		// Low-level shapes
		void draw_two_point_shape(int x1, int y1, int x2, int y2, uint16_t color, uint8_t reg, uint8_t cmd);  // drawLine, drawRect, fillRect
		void draw_three_point_shape(int x1, int y1, int x2, int y2, int x3, int y3, uint16_t color, uint8_t reg, uint8_t cmd);  // drawTriangle, fillTriangle
		void draw_ellipse_shape(int x, int y, int xrad, int yrad, uint16_t color, uint8_t cmd);  // drawCircle, fillCircle
	...
	public:
		...
		{
			this->draw_two_point_shape(x1, y1, x2, y2, color, RA8876_REG_DCR0, 0x80);
		};
		void drawRect(int x1, int y1, int x2, int y2, uint16_t color)
		{
			this->draw_two_point_shape(x1, y1, x2, y2, color, RA8876_REG_DCR1, 0xA0);
		};
		void fillRect(int x1, int y1, int x2, int y2, uint16_t color)
		{
			this->draw_two_point_shape(x1, y1, x2, y2, color, RA8876_REG_DCR1, 0xE0);
		};

		void drawTriangle(int x1, int y1, int x2, int y2, int x3, int y3, uint16_t color)
		{
			this->draw_three_point_shape(x1, y1, x2, y2, x3, y3, color, RA8876_REG_DCR0, 0xA2);
		};
		void fillTriangle(int x1, int y1, int x2, int y2, int x3, int y3, uint16_t color)
		{
			this->draw_three_point_shape(x1, y1, x2, y2, x3, y3, color, RA8876_REG_DCR0, 0xE2);
		};

		void drawCircle(int x, int y, int radius, uint16_t color)
		{
			this->draw_ellipse_shape(x, y, radius, radius, color, 0x80);
		};
		void fillCircle(int x, int y, int radius, uint16_t color)
		{
			this->draw_ellipse_shape(x, y, radius, radius, color, 0xC0);
		};
		...
		void set_backlight_intensity(const uint8_t intensity);
};
